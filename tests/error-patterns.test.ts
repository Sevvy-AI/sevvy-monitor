...
describe("Error Patterns", () => {
  describe("DEFAULT_ERROR_PATTERNS", () => {
    ...
    it("should match common error keywords", () => {
      const genericErrorPattern = DEFAULT_ERROR_PATTERNS.find(
        p => p.name === "Generic Error"
      );
      expect(genericErrorPattern).toBeDefined();

      expect(genericErrorPattern!.regex.test("An error occurred")).toBe(true);
      expect(
        genericErrorPattern!.regex.test("ERROR: Database connection failed")
      ).toBe(true);
      expect(genericErrorPattern!.regex.test("Error processing request")).toBe(
        true
      );
    ...
    it("should match HTTP error codes", () => {
      const httpErrorPattern = DEFAULT_ERROR_PATTERNS.find(
        p => p.name === "HTTP Error"
      );
      expect(httpErrorPattern).toBeDefined();

      expect(httpErrorPattern!.regex.test("404 not found")).toBe(true);
      expect(httpErrorPattern!.regex.test("500 internal server error")).toBe(
        true
      );
